...
const TravelPackages = () => {
  ...
  const fetchVideoPages = async () => {
    try {
      ...
      if (indianVideo) {
        if (indianVideo.mediaType === 'image' && indianVideo.backgroundImage) {
          setImageSrc(indianVideo.backgroundImage);
          setMediaType('image');
          setIsGoogleDriveVideo(false);
        } else if (indianVideo.backgroundVideo) {
          const videoUrl = indianVideo.backgroundVideo;
  ...
  // Fetch national packages from flipcard API
  const fetchNationalPackages = async () => {
    try {
      ...
      if (res.data && res.data.national) {
        setPackages(res.data.national);
      }
    } catch (error) {
      console.error("Error fetching national packages:", error);
    }
  };

  useEffect(() => {
    fetchVideoPages();
    fetchNationalPackages();
  }, []);
  ...
  return (
    <div className="h-screen mt-32 bg-white flex flex-col">
      {/* Video/Image Section */}
      <div className="relative w-full h-[32%]">
        ...
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center text-white">
          ...
          <h3 className="text-lg mt-2">
            Where Every<span className="text-yellow-400"> Journey </span> Tells A
