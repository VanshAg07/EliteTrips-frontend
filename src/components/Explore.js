    ...
    <div className="pb-20 pt-10  w-full">
      <div className="w-4/5 mx-auto">
        ...
        <div className="flex items-center justify-center relative py-2">
          <div className="flex gap-8 transition-transform duration-500 ease-in-out w-full justify-center">
            {adventures.length > 0 &&
              adventures.map((adventure) => (
                <div
                  key={adventure._id}
                  onClick={() => navigateToPage(adventure.route)}
                  className="w-1/3 max-w-sm relative cursor-pointer group"
                >
                  <img
                    src={adventure.image}
                    alt={adventure.title}
                    className="w-full h-[480px] object-cover rounded-2xl shadow-lg shadow-black transition-all duration-300 group-hover:scale-105"
