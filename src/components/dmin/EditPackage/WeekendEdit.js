...
function WeekendEdit() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5001/api/edit-packages/get-weekends-packages")
      .then((response) => {
        if (response.data) {
  ...
  const handleSubmit = (e) => {
    ...
    if (selectedTrip) {
      ...
      axios
        .put(
          `http://localhost:5001/api/edit-packages/edit-weekends-package/${selectedTrip.stateName}/${selectedTrip._id}`,
          tripDetails
        )
        .then((response) => {
  ...
  const handleDeleteTrip = (pkg, tripId) => {
    ...
    if (confirmed) {
      axios
        .delete(
          `http://localhost:5001/api/edit-packages/delete-weekends-package/${pkg.stateName}/${tripId}`
        )
        .then((response) => {
